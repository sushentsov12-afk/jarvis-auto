  ...
  .https.onCall(async (data, context) => {
    ...
    try {
      ...
      for (const subcoll of subcollections) {
        ...
        // Рекурсивно удалить вложенные subcollections
        const subcollDocs = await userRef.collection(subcoll).listDocuments();
        for (const subdoc of subcollDocs) {
          const subsubcollections = await subdoc.listCollections();
          for (const subsubcoll of subsubcollections) {
            const subsubdocs = await subsubcoll.listDocuments();
            for (const subsubdoc of subsubdocs) {
              await subsubdoc.delete();
            }
          }
          await subdoc.delete();
        }
