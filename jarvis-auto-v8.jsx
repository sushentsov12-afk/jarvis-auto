...
const REGION_OSAGO={"Москва":{coef:2.0,min:9000,max:44000},"Санкт-Петербург":{coef:1.8,min:8100,max:39600},"Московская область":{coef:1.7,min:7650,max:37400},"Краснодарский край":{coef:1.3,min:5850,max:28600},"Свердловская область":{coef:1.4,min:6300,max:30800},"Татарстан":{coef:1.3,min:5850,max:28600},"Новосибирская область":{coef:1.2,min:5400,max:26400},"Тюменская область":{coef:1.3,min:5850,max:28600},"default":{coef:1.0,min:4500,max:22000}};
...
/* ── HOOKS ───────────────────────────────────────────────────────── */
function usePersist(key,init){const[v,setV]=useState(()=>{try{const s=localStorage.getItem("ja8_"+key);return s?JSON.parse(s):init}catch{return init}});const set=useCallback(fn=>setV(prev=>{const next=typeof fn==="function"?fn(prev):fn;try{localStorage.setItem("ja8_"+key,JSON.stringify(next))}catch{}return next}),[key]);return[v,set];}
...
const FIND_SYS=`Создай 4 реалистичных демо-сервиса для указанного города. JSON:{"tip":"совет","results":[{"name":"","type":"","address":"","rating":4.5,"reviews":100,"price":"от 500 ₽","phone":"+7 (900) 000-00-00","hours":"9:00–21:00","wait":"~20 мин","tags":["быстро","гарантия"]}]}`;
...
function ProgressBar({remaining}){const pct=Math.max(0,Math.min(1,remaining));const c=pct>0.5?C.g:pct>0.2?C.y:C.r;return<div style={{height:4,background:"rgba(255,255,255,0.06)",borderRadius:2}}><div style={{height:4,background:c,borderRadius:2,width:`${pct*100}%`,transition:"width .7s cubic-bezier(.4,0,.2,1)"}}/></div>;}
function TipPopup({tip,onClose}){if(!tip)return null;return<div style={{position:"fixed",bottom:100,left:"50%",transform:"translateX(-50%)",width:"calc(100% - 32px)",maxWidth:480,zIndex:200,animation:"fadeUp .4s ease"}}><div style={{background:C.card2,border:`1px solid ${C.b}40`,borderRadius:14,padding:"16px 18px",boxShadow:`0 8px 32px rgba(0,0,0,.7)`}}><div style={{display:"flex",gap:12,alignItems:"flex-start"}}><span style={{fontSize:22,flexShrink:0}}>{tip.icon}</span><div style={{flex:1}}><div style={{fontSize:10,color:C.b,fontFamily:M,letterSpacing:1.5,marginBottom:6}}>💡 СОВЕТ ДЖАРВИСА</div><div style={{fontSize:13,color:C.t,fontFamily:S,lineHeight:1.6}}>{tip.tip}</div></div><button onClick={onClose} style={{background:"none",border:"none",color:C.t2,cursor:"pointer",fontSize:18,padding:"0 2px",lineHeight:1}}>×</button></div></div></div>;}

/* ── NOTIFICATION CENTER ─────────────────────────────────────────── */
function buildNotifications(services, docs, tires, appts, car) {
  const notifs = [];
  const now = new Date();

  // Service overdue/warning
  services.filter(s => s.status === "overdue").forEach(s => {
    const rem = s.lastKm + s.intervalKm - car.mileage;
    notifs.push({ id: `svc_${s.id}`, type: "overdue", icon: s.icon, title: s.name, body: `Просрочено на ${Math.abs(rem).toLocaleString()} км`, color: C.r, priority: 1 });
  });
  services.filter(s => s.status === "warning").forEach(s => {
    const rem = s.lastKm + s.intervalKm - car.mileage;
    notifs.push({ id: `svc_${s.id}`, type: "warning", icon: s.icon, title: s.name, body: `Осталось ~${rem.toLocaleString()} км`, color: C.y, priority: 2 });
  });
  ...
  docs.forEach(d => {
    ...
    if (st === "overdue") notifs.push({ id: `doc_${d.id}`, type: "overdue", icon: d.icon, title: d.name, body: `Просрочен на ${Math.abs(days)} дн.`, color: C.r, priority: 1 });
    else if (st === "warning") notifs.push({ id: `doc_${d.id}`, type: "warning", icon: d.icon, title: d.name, body: `Истекает через ${days} дн. (${d.expires})`, color: C.y, priority: 2 });
  });

  // Tire pressure
  Object.entries(tires).forEach(([key, t]) => {
    const st = tirePressureStatus(t.pressure);
    if (st.label === "КРИТИЧНО") notifs.push({ id: `tire_${key}`, type: "overdue", icon: "🛞", title: `Шина: ${t.pos}`, body: `Давление ${t.pressure} бар — критично`, color: C.r, priority: 1 });
    else if (st.label === "НИЗКОЕ") notifs.push({ id: `tire_${key}`, type: "warning", icon: "🛞", title: `Шина: ${t.pos}`, body: `Давление ${t.pressure} бар — низкое`, color: C.y, priority: 2 });
  });
  ...
  appts.filter(a => {
    ...
    const days = daysUntil(a.date);
    notifs.push({ id: `appt_${a.id}`, type: "info", icon: "📅", title: a.service, body: `${days === 0 ? "Сегодня" : `Через ${days} дн.`} · ${a.shop || "Сервис"}`, color: C.b, priority: 3 });
  });

  return notifs.sort((a, b) => a.priority - b.priority);
}

function NotificationCenter({ services, docs, tires, appts, car, setTab, onClose }) {
  const notifs = useMemo(() => buildNotifications(services, docs, tires, appts, car), [services, docs, tires, appts, car]);
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 300, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "80px 16px 16px" }}
      onClick={onClose}>
      <div style={{ width: "100%", maxWidth: 540, background: C.bg2, border: `1px solid ${C.border2}`, borderRadius: 16, overflow: "hidden", maxHeight: "80vh", overflowY: "auto", animation: "fadeUp .3s ease" }}
        onClick={e => e.stopPropagation()}>
        <div style={{ padding: "16px 18px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, background: C.bg2, zIndex: 1 }}>
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.t, fontFamily: S }}>Уведомления</div>
            <div style={{ fontSize: 11, color: C.t2, fontFamily: M }}>{notifs.length} активных</div>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: C.t2, cursor: "pointer", fontSize: 22, padding: "0 4px", lineHeight: 1 }}>×</button>
        </div>
        {notifs.length === 0 ? (
          <div style={{ textAlign: "center", padding: "48px 24px" }}>
            <div style={{ fontSize: 36, marginBottom: 12 }}>✓</div>
            <div style={{ fontSize: 15, fontWeight: 600, color: C.g, fontFamily: S, marginBottom: 4 }}>Всё в порядке</div>
            <div style={{ fontSize: 12, color: C.t2, fontFamily: S }}>Нет активных предупреждений</div>
          </div>
        ) : notifs.map((n, i) => (
          <div key={n.id} style={{ display: "flex", gap: 12, padding: "13px 18px", borderBottom: i < notifs.length - 1 ? `1px solid ${C.border}` : "none", alignItems: "center" }}>
            <div style={{ width: 36, height: 36, borderRadius: "50%", background: n.color + "20", border: `1px solid ${n.color}40`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 16 }}>{n.icon}</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, color: C.t, fontFamily: S, fontWeight: 500 }}>{n.title}</div>
              <div style={{ fontSize: 11, color: C.t2, fontFamily: S, marginTop: 2 }}>{n.body}</div>
            </div>
...
/* ── ANALYTICS ───────────────────────────────────────────────────── */
function Analytics({ expenses, fuel, services, car, diagHistory }) {
  const totalExp = expenses.reduce((s, e) => s + e.amount, 0);
  const totalKm = useMemo(() => {
    if (fuel.length < 2) return 0;
    const s = [...fuel].sort((a, b) => a.odometer - b.odometer);
    return s[s.length - 1].odometer - s[0].odometer;
  }, [fuel]);
  const costPerKm = totalKm > 0 ? (totalExp / totalKm).toFixed(1) : null;

  const avgCons = useMemo(() => {
    if (fuel.length < 2) return null;
    const s = [...fuel].sort((a, b) => a.odometer - b.odometer);
    let tL = 0, tK = 0;
    for (let i = 1; i < s.length; i++) { tL += s[i - 1].liters; tK += s[i].odometer - s[i - 1].odometer; }
  ...
  const monthly = useMemo(() => {
    const m = {};
    expenses.forEach(e => { const k = e.date.slice(0, 7); m[k] = (m[k] || 0) + e.amount; });
    return Object.entries(m).sort(([a], [b]) => a.localeCompare(b)).slice(-8).map(([mo, sum]) => ({ mo: mo.slice(5), sum }));
  }, [expenses]);

  // Service forecast — when is the next service due (in km)?
  const nextServices = useMemo(() => {
    return [...services]
      .map(s => {
        const kmRem = (s.lastKm + s.intervalKm) - car.mileage;
        const moRem = s.intervalMo - (Date.now() - new Date(s.lastDate)) / (1000 * 60 * 60 * 24 * 30);
        return { ...s, kmRem, moRem, dueKm: s.lastKm + s.intervalKm };
      })
      .sort((a, b) => a.kmRem - b.kmRem)
      .slice(0, 5);
  }, [services, car.mileage]);

  // Weekly km estimate (from fuel logs)
  const weeklyKmEst = useMemo(() => {
    if (fuel.length < 2) return null;
    const s = [...fuel].sort((a, b) => a.odometer - b.odometer);
    const firstDate = new Date(s[0].date), lastDate = new Date(s[s.length - 1].date);
    const weeks = (lastDate - firstDate) / (1000 * 60 * 60 * 24 * 7);
    const km = s[s.length - 1].odometer - s[0].odometer;
    return weeks > 0 ? Math.round(km / weeks) : null;
  }, [fuel]);
  ...
  const fuelTrend = useMemo(() => {
    ...
    const s = [...fuel].sort((a, b) => a.odometer - b.odometer);
    return s.slice(1).map((f, i) => {
      const km = f.odometer - s[i].odometer;
      return km > 0 ? { mo: f.date.slice(5), cons: ((s[i].liters / km) * 100).toFixed(1) } : null;
    }).filter(Boolean);
  }, [fuel]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <Card style={{ background: `${C.o}08`, borderColor: `${C.o}25` }}>
          <Lbl mb={4} color={C.o}>СТОИМОСТЬ / КМ</Lbl>
          <div style={{ fontSize: 26, fontWeight: 700, color: C.o, fontFamily: M }}>{costPerKm ? `${costPerKm} ₽` : "—"}</div>
          <div style={{ fontSize: 11, color: C.t2, fontFamily: S, marginTop: 3 }}>все расходы ÷ пробег</div>
        </Card>
        <Card style={{ background: `${C.b}08`, borderColor: `${C.b}25` }}>
          <Lbl mb={4} color={C.b}>РАСХОД ТОПЛИВА</Lbl>
          <div style={{ fontSize: 26, fontWeight: 700, color: C.b, fontFamily: M }}>{avgCons ? `${avgCons}` : "—"}</div>
          <div style={{ fontSize: 11, color: C.t2, fontFamily: S, marginTop: 3 }}>л / 100 км</div>
        </Card>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
        <Card style={{ padding: "12px 14px" }}>
          <Lbl mb={3} style={{ fontSize: 9 }}>ВСЕГО РАСХОДОВ</Lbl>
          <div style={{ fontSize: 14, fontWeight: 700, color: C.t, fontFamily: M }}>{rub(totalExp)}</div>
        </Card>
        <Card style={{ padding: "12px 14px" }}>
          <Lbl mb={3} style={{ fontSize: 9 }}>ЗАПИСЕЙ</Lbl>
          <div style={{ fontSize: 14, fontWeight: 700, color: C.t, fontFamily: M }}>{expenses.length}</div>
        </Card>
        <Card style={{ padding: "12px 14px" }}>
          <Lbl mb={3} style={{ fontSize: 9 }}>КМ В НЕДЕЛЮ</Lbl>
          <div style={{ fontSize: 14, fontWeight: 700, color: weeklyKmEst ? C.teal : C.t, fontFamily: M }}>{weeklyKmEst ? `~${weeklyKmEst}` : "—"}</div>
        </Card>
      </div>
      ...
      <Card>
        ...
        <ResponsiveContainer width="100%" height={140}>
          <BarChart data={monthly} barSize={28}>
            ...
            <Bar dataKey="sum" fill={C.o} radius={[5, 5, 0, 0]}>
              {monthly.map((_, i) => <Cell key={i} fill={i === monthly.length - 1 ? C.o : C.o + "80"} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </Card>
      ...
      {fuelTrend.length > 1 && (
        <Card>
          <Lbl>ТРЕНД РАСХОДА ТОПЛИВА</Lbl>
          <ResponsiveContainer width="100%" height={100}>
            <LineChart data={fuelTrend}>
              <XAxis dataKey="mo" tick={{ fontSize: 10, fill: C.t2, fontFamily: M }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 9, fill: C.t2, fontFamily: M }} axisLine={false} tickLine={false} domain={['auto', 'auto']} />
              <Tooltip content={({ active, payload }) => active && payload?.length ? <div style={{ background: C.card2, border: `1px solid ${C.border}`, borderRadius: 8, padding: "6px 10px", fontSize: 11, fontFamily: M, color: C.t }}>{payload[0].value} л/100км</div> : null} />
              <Line type="monotone" dataKey="cons" stroke={C.teal} strokeWidth={2.5} dot={{ fill: C.teal, r: 4 }} activeDot={{ r: 6 }} />
            </LineChart>
          </ResponsiveContainer>
        </Card>
      )}
              ...
              <div key={name} style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 7 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 12, color: C.t, fontFamily: S, textTransform: "capitalize" }}>{name}</span>
                    <span style={{ fontSize: 12, color: C.t, fontFamily: M }}>{rub(value)}</span>
                  </div>
                  <div style={{ height: 2, background: C.border, borderRadius: 1, marginTop: 3 }}>
                    <div style={{ height: 2, background: CAT_CLR[name] || C.t2, borderRadius: 1, width: `${(value / totalExp) * 100}%` }} />
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* Service forecast */}
      <Card>
        <Lbl>ПРОГНОЗ БЛИЖАЙШИХ ТО</Lbl>
        {nextServices.map((s, i) => (
          <div key={s.id} style={{ display: "flex", gap: 10, padding: "10px 0", borderBottom: i < nextServices.length - 1 ? `1px solid ${C.border}` : "none", alignItems: "center" }}>
            <span style={{ fontSize: 18, flexShrink: 0 }}>{s.icon}</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, color: C.t, fontFamily: S, fontWeight: 500 }}>{s.name}</div>
              <div style={{ marginTop: 4 }}><ProgressBar remaining={Math.max(0, s.kmRem) / s.intervalKm} /></div>
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 3 }}>
                <div style={{ fontSize: 10, color: C.t2, fontFamily: M }}>через {s.dueKm.toLocaleString()} км</div>
                <div style={{ fontSize: 10, color: s.kmRem < 0 ? C.r : s.kmRem < s.intervalKm * 0.15 ? C.y : C.g, fontFamily: M }}>
                  {s.kmRem < 0 ? `−${Math.abs(s.kmRem).toLocaleString()} км` : `${s.kmRem.toLocaleString()} км`}
                </div>
              </div>
            </div>
          </div>
        ...
        {weeklyKmEst && (
          <div style={{ marginTop: 10, padding: "10px 12px", background: `${C.teal}10`, borderRadius: 8, border: `1px solid ${C.teal}25` }}>
            <div style={{ fontSize: 11, color: C.teal, fontFamily: S }}>
              💡 При темпе <b>{weeklyKmEst} км/нед</b> — {nextServices[0]?.name} потребуется через ~<b>{Math.max(0, Math.round(nextServices[0]?.kmRem / weeklyKmEst || 0))} нед.</b>
            </div>
          </div>
        )}
      </Card>
      ...
      {diagHistory.length > 0 && (
        <Card>
          <Lbl>ДИАГНОЗЫ ПО ОПАСНОСТИ</Lbl>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
            {[["низкий", C.g, "НИЗКИЙ"], ["средний", C.y, "СРЕДНИЙ"], ["высокий", C.r, "ВЫСОКИЙ"]].map(([level, color, label]) => {
              const cnt = diagHistory.filter(d => d.danger === level).length;
              return (
                <div key={level} style={{ textAlign: "center", padding: "10px", background: color + "10", borderRadius: 10, border: `1px solid ${color}30` }}>
                  <div style={{ fontSize: 20, fontWeight: 700, color, fontFamily: M }}>{cnt}</div>
                  <div style={{ fontSize: 9, color: C.t2, fontFamily: M, letterSpacing: 1 }}>{label}</div>
                </div>
              );
            })}
          </div>
...
function QuickAdd({ onAdd, onClose }) {
  const [cat, setCat] = useState("топливо");
  const [amount, setAmount] = useState("");
  const [note, setNote] = useState("");
  const presets = { топливо: [1500, 2000, 2500, 3000, 3500], сервис: [1000, 2500, 5000, 10000], запчасти: [500, 1500, 3000, 8000] };
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 300, display: "flex", alignItems: "flex-end", justifyContent: "center" }}
      onClick={onClose}>
      <div style={{ width: "100%", maxWidth: 640, background: C.bg2, border: `1px solid ${C.border2}`, borderRadius: "16px 16px 0 0", padding: "20px 18px 32px", animation: "slideUp .3s ease" }}
        onClick={e => e.stopPropagation()}>
        <style>{`@keyframes slideUp{from{transform:translateY(100%)}to{transform:translateY(0)}}`}</style>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: C.t, fontFamily: S }}>Быстрый расход</div>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 14, overflowX: "auto" }}>
          {Object.keys(CAT_CLR).map(c => (
            <button key={c} onClick={() => setCat(c)} style={{ padding: "5px 12px", fontSize: 11, fontFamily: M, background: cat === c ? (CAT_CLR[c] || C.t2) + "25" : "transparent", color: cat === c ? CAT_CLR[c] : C.t2, border: `1px solid ${cat === c ? (CAT_CLR[c] || C.t2) + "60" : C.border}`, borderRadius: 6, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0, textTransform: "capitalize" }}>{c}</button>
          ))}
        </div>
        {/* Presets */}
        {presets[cat] && (
          <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
            {presets[cat].map(v => (
              <button key={v} onClick={() => setAmount(String(v))} style={{ padding: "5px 12px", fontSize: 11, fontFamily: M, background: amount === String(v) ? C.oG : "transparent", color: amount === String(v) ? C.o : C.t2, border: `1px solid ${amount === String(v) ? C.o + "60" : C.border}`, borderRadius: 6, cursor: "pointer", flexShrink: 0 }}>{rub(v)}</button>
            ))}
          </div>
        )}
        {/* Inputs */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
          <div>
            <Lbl mb={5}>СУММА (₽)</Lbl>
            <Input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="3200" autoFocus style={{ fontSize: 18, fontWeight: 700 }} />
          </div>
          <div>
            <Lbl mb={5}>ПРИМЕЧАНИЕ</Lbl>
            <Input value={note} onChange={e => setNote(e.target.value)} placeholder="АЗС Лукойл" onKeyDown={e => e.key === "Enter" && amount && onAdd({ cat, amount: +amount, note, date: new Date().toISOString().slice(0, 10), id: Date.now() })} />
          </div>
        </div>
        <PBtn full onClick={() => { if (!amount) return; onAdd({ cat, amount: +amount, note, date: new Date().toISOString().slice(0, 10), id: Date.now() }); }} disabled={!amount} style={{ padding: "14px", fontSize: 14 }}>
          ДОБАВИТЬ {amount ? rub(+amount) : ""}
        </PBtn>
      </div>
    </div>
  );
...
/* ── DASHBOARD ───────────────────────────────────────────────────── */
function Dashboard({ name, car, services, diagHistory, expenses, fuel, body, tires, geo, docs, appts, setTab, toast, onRequestGeo, geoLoading, onQuickAdd, onOpenNotifs, notifCount }) {
  ...
  // Insurance countdown
  const osago = docs.find(d => d.name === "ОСАГО");
  const osagoExpiry = osago ? daysUntil(osago.expires) : null;

  const weekly = useMemo(() => [4, 3, 2, 1].map(w => {
    const now = new Date();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        ...
        {/* Notification bell */}
        <button onClick={onOpenNotifs} style={{ position: "relative", background: notifCount ? `${C.r}15` : C.card, border: `1px solid ${notifCount ? C.r + "40" : C.border}`, borderRadius: 10, width: 40, height: 40, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
          <span style={{ fontSize: 18 }}>🔔</span>
          {notifCount > 0 && <div style={{ position: "absolute", top: -4, right: -4, minWidth: 16, height: 16, borderRadius: 8, background: C.r, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 3px", border: `2px solid ${C.bg}` }}><span style={{ fontSize: 8, color: "white", fontWeight: 700, fontFamily: M }}>{notifCount > 9 ? "9+" : notifCount}</span></div>}
        </button>
      </div>
      ...
      <Card style={{ background: `linear-gradient(135deg,${C.card} 0%,#0f1f3a 100%)`, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", gap: 18, alignItems: "center" }}>
          <Ring val={health} color={hColor} size={96} sw={8}>
            <div style={{ fontSize: 20, fontWeight: 700, color: hColor, fontFamily: M, lineHeight: 1 }}>{health}</div>
            <div style={{ fontSize: 8, color: C.t2, letterSpacing: .5 }}>/ 100</div>
          </Ring>
          <div style={{ flex: 1 }}>
            <Lbl mb={3}>ЗДОРОВЬЕ АВТО</Lbl>
            <div style={{ fontSize: 18, fontWeight: 700, color: hColor, fontFamily: M, marginBottom: 3 }}>{health >= 75 ? "ОТЛИЧНО" : health >= 50 ? "ВНИМАНИЕ" : "КРИТИЧНО"}</div>
            <div style={{ fontSize: 11, color: C.t2, fontFamily: S, lineHeight: 1.5, marginBottom: 6 }}>{overdue.length > 0 ? `${overdue.length} операций просрочено` : warning.length > 0 ? `${warning.length} скоро истекут` : "Всё в норме"}</div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={onQuickAdd} style={{ flex: 1, padding: "10px 14px", background: C.o, color: "white", border: "none", borderRadius: 10, fontSize: 11, fontFamily: M, fontWeight: 700, cursor: "pointer", letterSpacing: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
          <span style={{ fontSize: 15 }}>+</span> ДОБАВИТЬ РАСХОД
        </button>
        <button onClick={() => setTab("diag")} style={{ flex: 1, padding: "10px 14px", background: C.card2, color: C.t, border: `1px solid ${C.border}`, borderRadius: 10, fontSize: 11, fontFamily: M, cursor: "pointer", letterSpacing: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
          <span style={{ fontSize: 15 }}>⚙</span> ДИАГНОСТИКА
        </button>
      </div>

      {/* 5 stats */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 6 }}>
        {[{ l: "РАСХОДЫ", v: rub(totalExp), c: C.t, tab: "analytics" }, { l: "СЕРВИС ТО", v: overdue.length, c: overdue.length ? C.r : C.g, tab: "maintain" }, { l: "КУЗОВ", v: bodyDamaged, c: bodyDamaged ? C.y : C.g, tab: "body" }, { l: "ШИНЫ", v: tiresOk ? "OK" : "!", c: tiresOk ? C.g : C.y, tab: "more" }, { l: "ЗАПИСИ", v: upcomingAppts, c: upcomingAppts ? C.b : C.t, tab: "appts" }].map(({ l, v, c, tab }) => (
          <Card key={l} onClick={() => setTab(tab)} style={{ padding: "8px 6px", cursor: "pointer", textAlign: "center" }}>
            <Lbl mb={2} style={{ fontSize: 7, letterSpacing: 1 }}>{l}</Lbl>
            <div style={{ fontSize: 12, fontWeight: 700, color: c, fontFamily: M }}>{v}</div>
          </Card>
        ))}
      </div>

      {/* OSAGO insurance countdown */}
      {osagoExpiry !== null && (
        <Card style={{
          background: osagoExpiry < 0 ? `${C.r}08` : osagoExpiry < 30 ? `${C.y}08` : `${C.g}08`,
          borderColor: osagoExpiry < 0 ? `${C.r}30` : osagoExpiry < 30 ? `${C.y}30` : `${C.g}30`,
          cursor: "pointer"
        }} onClick={() => setTab("more")}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <div style={{ width: 40, height: 40, borderRadius: "50%", background: (osagoExpiry < 0 ? C.r : osagoExpiry < 30 ? C.y : C.g) + "20", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>📄</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: C.t, fontFamily: S }}>ОСАГО</div>
              <div style={{ fontSize: 11, color: osagoExpiry < 0 ? C.r : osagoExpiry < 30 ? C.y : C.g, fontFamily: M }}>
                {osagoExpiry < 0 ? `⚠ Просрочен на ${Math.abs(osagoExpiry)} дн.` : osagoExpiry === 0 ? "⚠ Истекает сегодня!" : osagoExpiry < 30 ? `⚠ Истекает через ${osagoExpiry} дн.` : `Действует ещё ${osagoExpiry} дн.`}
              </div>
            </div>
            {geo?.region && <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 9, color: C.t2, fontFamily: M }}>КБМ РЕГИОН</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: C.p, fontFamily: M }}>{regionOsago.coef}x</div>
            </div>}
          </div>
        </Card>
      )}
      ...
      {!geo?.region && (
        <Card style={{ background: `${C.b}06`, borderColor: `${C.b}20`, cursor: "pointer" }} onClick={onRequestGeo}>
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 12, color: C.b, fontFamily: M }}>{geoLoading ? "ОПРЕДЕЛЯЮ..." : "ВКЛЮЧИТЬ ГЕОЛОКАЦИЮ"}</div>
              <div style={{ fontSize: 11, color: C.t2, fontFamily: S }}>ОСАГО по региону и сервисы рядом</div>
            </div>
          </div>
        </Card>
      )}
      ...
      {(overdue.length + warning.length > 0) && (
        <Card style={{ background: `${C.r}05`, borderColor: `${C.r}20` }}>
          <Lbl color={C.r}>⚠ НУЖЕН СЕРВИС</Lbl>
          {[...overdue, ...warning].slice(0, 3).map((s, i, arr) => (
      ...
      {/* Body damage alert */}
      {needRepair > 0 && (
        <Card style={{ background: `${C.o}06`, borderColor: `${C.o}25`, cursor: "pointer" }} onClick={() => setTab("body")}>
          <Lbl color={C.o} mb={4}>🚗 КУЗОВ — {needRepair} зон требуют ремонта</Lbl>
          <div style={{ fontSize: 11, color: C.t2, fontFamily: S }}>Открыть схему кузова →</div>
        </Card>
      )}
        ...
        <Card>
          ...
          {diagHistory.slice(0, 3).map((d, i) => (
            <div key={i} style={{ display: "flex", gap: 10, padding: "9px 0", borderBottom: i < 2 ? `1px solid ${C.border}` : "none", alignItems: "center" }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, color: C.t, fontFamily: S }}>{d.title}</div>
                <div style={{ fontSize: 10, color: C.t2, fontFamily: M }}>{d.query} · {d.time}</div>
              </div>
...
function Maintain({ services, setServices, car, setCar, toast }) {
  ...
  const saveMileage = () => {
    ...
    setServices(prev => recalcAll(prev, newKm));
    setEditKm(false);
    toast(`Пробег: ${newKm.toLocaleString()} км · ТО пересчитано`, "success", 4000);
  };
  ...
  const markDone = id => {
    ...
    setServices(prev => recalcAll(prev.map(s => {
      if (s.id !== id) return s;
      return { ...s, lastKm: car.mileage, lastDate: now, history: [{ date: now, km: car.mileage, note: "Плановая замена" }, ...(s.history || [])].slice(0, 10) };
    }), car.mileage));
  ...
  ].sort((a, b) => ({ overdue: 0, warning: 1, ok: 2 })[a.status] - ({ overdue: 0, warning: 1, ok: 2 })[b.status]);

  // Service history list
  const allHistory = useMemo(() => {
    const items = [];
    services.forEach(svc => {
      (svc.history || []).forEach(h => { items.push({ ...h, svcName: svc.name, svcIcon: svc.icon }); });
      if (svc.lastDate && svc.lastKm) items.push({ date: svc.lastDate, km: svc.lastKm, note: "Последняя замена", svcName: svc.name, svcIcon: svc.icon });
    });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {/* Search */}
      <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍  Поиск по названию…" style={{ background: C.card }} />
      ...
      {showHistory ? (
        <Card>
          ...
          {allHistory.length === 0 ? <div style={{ textAlign: "center", padding: "24px", color: C.t2, fontSize: 13, fontFamily: S }}>История пуста</div> :
            allHistory.map((h, i) => (
              <div key={i} style={{ display: "flex", gap: 12, padding: "10px 0", borderBottom: i < allHistory.length - 1 ? `1px solid ${C.border}` : "none", alignItems: "flex-start" }}>
        ...
        <Card>
          <Lbl>РЕГЛАМЕНТ — {sorted.length} ПОЗИЦИЙ{search ? ` (фильтр: "${search}")` : ""}</Lbl>
          {sorted.length === 0 && <div style={{ textAlign: "center", padding: "24px", color: C.t2, fontSize: 13, fontFamily: S }}>Ничего не найдено</div>}
          {sorted.map((s, i) => {
            ...
            return (
              <div key={s.id} style={{ padding: "12px 0", borderBottom: i < sorted.length - 1 ? `1px solid ${C.border}` : "none" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: s.status !== "ok" ? 8 : 0 }}>
                  <div style={{ fontSize: 10, color: C.t2, fontFamily: M }}>{s.lastDate} · {s.lastKm?.toLocaleString()} км · каждые {s.intervalKm?.toLocaleString()} км</div>
                ...
                {(s.history || []).length > 0 && <div style={{ fontSize: 10, color: C.t2, fontFamily: S, marginTop: 3 }}>📋 {(s.history || []).length} замен · последняя: {(s.history || [])[0]?.date}</div>}
              </div>
            );
          })}
        </Card>
...
function Diagnostics({ history, setHistory, car, toast }) {
  const [input, setInput] = useState(""); const [loading, setLoading] = useState(false); const [result, setResult] = useState(null); const [err, setErr] = useState(""); const [offline, setOffline] = useState(false); const [search, setSearch] = useState("");
  const ref = useRef(null);
  const examples = ["P0301", "P0420", "P0171", "P0300", "стук при торможении", "не заводится", "вибрация 80 км/ч"];
  const run = async (q) => {
    ...
    try { const text = await ai(DIAG_SYS, `Авто: ${car.make} ${car.model} ${car.year}, ${car.mileage?.toLocaleString()} км. ${query}`); const r = JSON.parse(text); if (r.error) setErr(r.error); else { setResult(r); setHistory(prev => [{ ...r, query, time: new Date().toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit" }) }, ...prev].slice(0, 30)); toast("Диагноз получен", "success"); } } catch { setErr("Ошибка соединения."); }
    setLoading(false);
  };

  const filteredHistory = history.filter(d => !search || d.query?.toLowerCase().includes(search.toLowerCase()) || d.title?.toLowerCase().includes(search.toLowerCase()));
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <Card>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
          <input ref={ref} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && run()} placeholder="P0301 или «стучит при разгоне»" style={{ flex: 1, padding: "11px 14px", fontSize: 13, fontFamily: M, background: C.bg2, border: `1px solid ${input ? C.o + "60" : C.border}`, borderRadius: 9, color: C.t, outline: "none", transition: "border-color .2s" }} />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, animation: "fadeUp .35s ease" }}>
          <Card style={{ borderLeft: `3px solid ${C.o}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              ...
              <div style={{ display: "flex", gap: 6 }}>{offline && <Pill color={C.g}>⚡ ОФЛАЙН</Pill>}{result.urgency && <Pill color={urgClr[result.urgency] || C.t2}>{result.urgency}</Pill>}</div>
            </div>
      ...
      {!result && !loading && (
        <>
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍  Поиск в истории…" style={{ background: C.card }} />
          {filteredHistory.length > 0 && (
            <Card>
              <Lbl>ИСТОРИЯ ({history.length}){search ? ` · найдено: ${filteredHistory.length}` : ""}</Lbl>
              {filteredHistory.slice(0, 10).map((d, i) => (
                <div key={i} onClick={() => { setInput(d.query); setTimeout(() => run(d.query), 20); }}
                  style={{ display: "flex", gap: 10, padding: "9px 4px", borderBottom: i < Math.min(10, filteredHistory.length) - 1 ? `1px solid ${C.border}` : "none", cursor: "pointer", borderRadius: 6, margin: "0 -4px" }}
...
function BodyMap({ body, setBody, car, toast }) {
  ...
  const damaged = BODY_PANELS.filter(p => body[p.id]?.status !== "ok").length;
  const needRepair = BODY_PANELS.filter(p => body[p.id]?.status === "repair").length;
  const handleClick = pid => {
    if (selected === pid && !noteEdit) { const next = nextStatus(body[pid]?.status || "ok"); setBody(b => ({ ...b, [pid]: { ...b[pid], status: next } })); toast(`${BODY_PANELS.find(p => p.id === pid)?.name}: ${PS[next].label}`, "info"); }
    else { setSelected(selected === pid ? null : pid); setNoteEdit(false); }
  };
  const selPanel = selected ? BODY_PANELS.find(p => p.id === selected) : null;
  const selState = selected ? body[selected] || { status: "ok", note: "" } : null;
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          ...
          <svg viewBox="0 0 200 390" style={{ width: "100%", maxWidth: 280, height: "auto", cursor: "pointer", userSelect: "none" }}>
            {BODY_PANELS.map(panel => {
              ...
              return <g key={panel.id}>
                ...
                {!panel.wheel && st !== "ok" && (() => { const bb = panel.path.split(/[MLZ\s,]+/).filter(Boolean).map(Number).filter(n => !isNaN(n)); const xs = bb.filter((_, i) => i % 2 === 0), ys = bb.filter((_, i) => i % 2 === 1); return <text x={(Math.min(...xs) + Math.max(...xs)) / 2} y={(Math.min(...ys) + Math.max(...ys)) / 2 + 4} textAnchor="middle" fontSize="9" fill={cfg.color} fontFamily="sans-serif" style={{ pointerEvents: "none" }}>{cfg.icon}</text>; })()}
              </g>;
            })}
      ...
      {selPanel && selState && (
        <Card style={{ borderLeft: `3px solid ${PS[selState.status].color}`, animation: "fadeUp .2s ease" }}>
      ...
      {damaged > 0 && (
        <Card>
          <Lbl>ПОВРЕЖДЁННЫЕ ЗОНЫ ({damaged})</Lbl>
          {BODY_PANELS.filter(p => body[p.id]?.status !== "ok").map((p, i, arr) => {
            const st = body[p.id]?.status || "ok"; const cfg = PS[st];
            return <div key={p.id} onClick={() => { setSelected(p.id); setNoteEdit(false); }} style={{ display: "flex", gap: 10, padding: "9px 0", borderBottom: i < arr.length - 1 ? `1px solid ${C.border}` : "none", cursor: "pointer", alignItems: "center" }} onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,.02)"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
              ...
              <div style={{ flex: 1 }}><div style={{ fontSize: 13, color: C.t, fontFamily: S }}>{p.name}</div>{body[p.id]?.note && <div style={{ fontSize: 11, color: C.t2, fontFamily: S }}>{body[p.id].note}</div>}</div>
...
function Finance({ expenses, setExpenses, fuel, setFuel, car, toast }) {
  const [view, setView] = useState("exp"); const [adding, setAdding] = useState(false); const [search, setSearch] = useState("");
  ...
  const filtered = expenses.filter(e => !search || e.note?.toLowerCase().includes(search.toLowerCase()) || e.cat?.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ display: "flex", gap: 6 }}>{[["exp", "РАСХОДЫ"], ["fuel", "ТОПЛИВО"]].map(([v, l]) => <button key={v} onClick={() => setView(v)} style={{ flex: 1, padding: "8px", fontSize: 10, fontFamily: M, borderRadius: 8, cursor: "pointer", letterSpacing: 1, border: `1px solid ${view === v ? C.o : C.border}`, background: view === v ? C.oG : "transparent", color: view === v ? C.o : C.t2 }}>{l}</button>)}</div>
      {view === "exp" && <>
        ...
        <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍  Поиск по расходам…" style={{ background: C.card }} />
        <Card><Lbl>ЗАПИСИ ({filtered.length}){search ? ` · фильтр: "${search}"` : ""}</Lbl>
          {filtered.map((e, i) => <div key={e.id} style={{ display: "flex", gap: 10, padding: "10px 0", borderBottom: i < filtered.length - 1 ? `1px solid ${C.border}` : "none", alignItems: "center" }}>
            ...
            <button onClick={() => setExpenses(p => p.filter(x => x.id !== e.id))} style={{ background: "none", border: "none", color: C.t2, cursor: "pointer", fontSize: 15, padding: "0 4px" }}>×</button>
          </div>)}
        </Card>
      </>}
      {view === "fuel" && <>
        ...
        {adding ? <Card><Lbl>НОВАЯ ЗАПРАВКА</Lbl>
          {[["ДАТА", "date", "date", ""], ["ЛИТРОВ", "liters", "number", "45"], ["СТОИМОСТЬ (₽)", "cost", "number", "3200"], ["ПРОБЕГ (КМ)", "odometer", "number", ""], ["АЗС", "station", "text", ""]].map(([l, k, t, ph]) => <div key={k} style={{ marginBottom: 10 }}><div style={{ fontSize: 10, color: C.t2, letterSpacing: 2, marginBottom: 5, fontFamily: M }}>{l}</div><Input type={t} value={fForm[k]} placeholder={ph} onChange={e => setFForm(p => ({ ...p, [k]: e.target.value }))} /></div>)}
        ...
        <Card><Lbl>ИСТОРИЯ</Lbl>
          {fuel.map((f, i) => <div key={f.id} style={{ display: "flex", gap: 10, padding: "10px 0", borderBottom: i < fuel.length - 1 ? `1px solid ${C.border}` : "none", alignItems: "center" }}>
            <span style={{ fontSize: 18 }}>⛽</span>
            ...
            <button onClick={() => setFuel(p => p.filter(x => x.id !== f.id))} style={{ background: "none", border: "none", color: C.t2, cursor: "pointer", fontSize: 15, padding: "0 4px" }}>×</button>
          </div>)}
        </Card>
      </>}
    </div>
...
/* ── MORE ────────────────────────────────────────────────────────── */
function More({ car, name, toast, setCar, expenses, setExpenses, fuel, setFuel, body, geo, onRequestGeo, geoLoading, appts, setAppts, tires, setTires, setServices, services, docs }) {
  const [sub, setSub] = useState(null);
  const TIPS_SYS2 = `Ты — Джарвис, автоэксперт-друг. Русский язык. Дружелюбно, конкретно, 3-4 абзаца.`;
  const FIND_SYS2 = `Создай 4 реалистичных демо-сервиса для указанного города. JSON:{"tip":"совет","results":[{"name":"","type":"","address":"","rating":4.5,"reviews":100,"price":"от 500 ₽","phone":"+7 (900) 000-00-00","hours":"9:00–21:00","wait":"~20 мин","tags":["быстро"]}]}`;
  const regionOsago = useMemo(() => getRegionOsago(geo?.region), [geo]);
  ...
  if (sub === "profile") return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <Card>
        {[["МАРКА", "make", "text", "Toyota"], ["МОДЕЛЬ", "model", "text", "Camry"], ["ГОД", "year", "number", "2019"], ["ПРОБЕГ (КМ)", "mileage", "number", "52000"], ["ЦВЕТ", "color", "text", "Белый перламутр"]].map(([l, k, t, ph]) => <div key={k} style={{ marginBottom: 12 }}><Lbl mb={5}>{l}</Lbl><Input type={t} value={car[k] || ""} placeholder={ph} onChange={e => setCar(p => ({ ...p, [k]: t === "number" ? +e.target.value : e.target.value }))} /></div>)}
        <div style={{ background: C.bg2, border: `1px solid ${C.border2}`, borderRadius: 10, padding: "14px", marginBottom: 12 }}>
          ...
          <div><Lbl mb={5}>VIN-КОД (17 знаков)</Lbl><Input value={car.vin || ""} placeholder="XTA..." onChange={e => setCar(p => ({ ...p, vin: fmtVin(e.target.value) }))} maxLength={17} style={{ fontFamily: M, fontSize: 11 }} />{car.vin?.length > 0 && <div style={{ fontSize: 10, color: car.vin.length === 17 ? C.g : C.y, fontFamily: M, marginTop: 5 }}>{car.vin.length === 17 ? "✓ VIN корректен" : `${car.vin.length}/17`}</div>}</div>
        </div>
  ...
  if (sub === "docs") {
    const [docsList, setDocsList] = usePersist("docs", DEF_DOCS);
    const [adding, setAdding] = useState(false);
    const [form, setForm] = useState({ name: "", icon: "📄", expires: "", note: "" });
    const updated = docsList.map(d => ({ ...d, status: docStatus(d.expires) }));
    const expiring = updated.filter(d => d.status !== "ok");
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <BackHeader title="Мои документы" onBack={() => setSub(null)} />
        {/* Regional OSAGO */}
        <Card style={{ background: `${C.p}08`, borderColor: `${C.p}25` }}>
          <Lbl color={C.p} mb={6}>ОСАГО — {geo?.region || "ВАШИ ДАННЫЕ"}</Lbl>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 6 }}>
            <div><div style={{ fontSize: 9, color: C.t2, fontFamily: M, marginBottom: 2 }}>КБМ КОЭФ.</div><div style={{ fontSize: 18, fontWeight: 700, color: C.p, fontFamily: M }}>{regionOsago.coef}x</div></div>
            <div><div style={{ fontSize: 9, color: C.t2, fontFamily: M, marginBottom: 2 }}>ОТ</div><div style={{ fontSize: 14, fontWeight: 700, color: C.t, fontFamily: M }}>{rub(regionOsago.min)}</div></div>
            <div><div style={{ fontSize: 9, color: C.t2, fontFamily: M, marginBottom: 2 }}>ДО</div><div style={{ fontSize: 14, fontWeight: 700, color: C.t, fontFamily: M }}>{rub(regionOsago.max)}</div></div>
          </div>
          {!geo?.region && <div style={{ fontSize: 11, color: C.b, fontFamily: S }}>💡 Включите геолокацию для ставок вашего региона</div>}
        </Card>
        ...
        <Card><Lbl>ВСЕ ДОКУМЕНТЫ</Lbl>
          {updated.map((d, i) => { const days = daysUntil(d.expires); const cfg = sCfg[d.status]; return <div key={d.id} style={{ display: "flex", gap: 10, padding: "12px 0", borderBottom: i < updated.length - 1 ? `1px solid ${C.border}` : "none", alignItems: "center" }}><span style={{ fontSize: 22, flexShrink: 0 }}>{d.icon}</span><div style={{ flex: 1 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ fontSize: 14, color: C.t, fontFamily: S, fontWeight: 500 }}>{d.name}</span><Pill color={cfg.color}>{cfg.icon} {cfg.label}</Pill></div><div style={{ fontSize: 11, color: C.t2, fontFamily: M }}>{d.note} · до {d.expires}</div><div style={{ fontSize: 11, color: cfg.color, fontFamily: M, marginTop: 2 }}>{days < 0 ? `Просрочен на ${Math.abs(days)} дн.` : days < 30 ? `Осталось ${days} дн.` : `${days} дней`}</div></div><button onClick={() => setDocsList(p => p.filter(x => x.id !== d.id))} style={{ background: "none", border: "none", color: C.t2, cursor: "pointer", fontSize: 14 }}>×</button></div>; })}
        </Card>
        {adding ? <Card><Lbl>НОВЫЙ ДОКУМЕНТ</Lbl>
          {[["НАЗВАНИЕ", "name", "text", "ОСАГО"], ["СРОК ДЕЙСТВИЯ", "expires", "date", ""], ["ПРИМЕЧАНИЕ", "note", "text", "Страховая"]].map(([l, k, t, ph]) => <div key={k} style={{ marginBottom: 10 }}><Lbl mb={5}>{l}</Lbl><Input type={t} value={form[k]} placeholder={ph} onChange={e => setForm(p => ({ ...p, [k]: e.target.value }))} /></div>)}
          <div style={{ display: "flex", gap: 8 }}><PBtn full onClick={() => { if (!form.name || !form.expires) return; setDocsList(p => [...p, { ...form, id: Date.now(), status: docStatus(form.expires) }]); setAdding(false); toast("Добавлено", "success"); }}>ДОБАВИТЬ</PBtn><GBtn onClick={() => setAdding(false)}>ОТМЕНА</GBtn></div>
  ...
  if (sub === "appts") {
    ...
    const svcList = ["Замена масла", "Диагностика", "Шиномонтаж", "ТО плановое", "Ремонт тормозов", "Кузовной ремонт", "Другое"];
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <BackHeader title="Записи в сервис" onBack={() => setSub(null)} />
        {upcoming.length > 0 && <Card style={{ background: `${C.g}06`, borderColor: `${C.g}20` }}>
          ...
          {upcoming.map((a, i) => <div key={a.id} style={{ display: "flex", gap: 10, padding: "10px 0", borderBottom: i < upcoming.length - 1 ? `1px solid ${C.border}` : "none", alignItems: "flex-start" }}>
            <div style={{ minWidth: 42, textAlign: "center" }}><div style={{ fontSize: 16, fontWeight: 700, color: C.g, fontFamily: M, lineHeight: 1 }}>{new Date(a.date).getDate()}</div><div style={{ fontSize: 10, color: C.t2, fontFamily: M }}>{new Date(a.date).toLocaleString("ru", { month: "short" })}</div></div>
            <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 600, color: C.t, fontFamily: S }}>{a.service}</div><div style={{ fontSize: 11, color: C.t2, fontFamily: S }}>{a.shop || "Сервис"}{a.time ? ` · ${a.time}` : ""}</div>{a.phone && <div style={{ fontSize: 11, color: C.b, fontFamily: M }}>{a.phone}</div>}{a.note && <div style={{ fontSize: 11, color: C.t2, fontFamily: S }}>📝 {a.note}</div>}</div>
            <button onClick={() => setAppts(p => p.filter(x => x.id !== a.id))} style={{ background: "none", border: "none", color: C.t2, cursor: "pointer", fontSize: 15 }}>×</button>
          </div>)}
        </Card>}
        {adding ? <Card><Lbl>НОВАЯ ЗАПИСЬ</Lbl>
          <div style={{ marginBottom: 12 }}><Lbl mb={7}>ВИД РАБОТЫ</Lbl><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{svcList.map(s => <button key={s} onClick={() => setForm(p => ({ ...p, service: s }))} style={{ padding: "5px 12px", fontSize: 11, fontFamily: S, background: form.service === s ? C.oG : "transparent", color: form.service === s ? C.o : C.t2, border: `1px solid ${form.service === s ? C.o + "60" : C.border}`, borderRadius: 5, cursor: "pointer" }}>{s}</button>)}</div></div>
          {[["ДАТА", "date", "date", ""], ["ВРЕМЯ", "time", "time", ""], ["НАЗВАНИЕ СЕРВИСА", "shop", "text", "Автосервис Мастер"], ["ТЕЛЕФОН", "phone", "tel", "+7 (900) 000-00-00"], ["ЗАМЕТКА", "note", "text", "Взять с собой..."]].map(([l, k, t, ph]) => <div key={k} style={{ marginBottom: 10 }}><Lbl mb={5}>{l}</Lbl><Input type={t} value={form[k]} placeholder={ph} onChange={e => setForm(p => ({ ...p, [k]: e.target.value }))} /></div>)}
          <div style={{ display: "flex", gap: 8 }}><PBtn full onClick={() => { if (!form.date || !form.service) return; setAppts(p => [{ ...form, id: Date.now() }, ...p]); setAdding(false); setForm({ date: "", time: "", service: "", shop: "", phone: "", note: "" }); toast("Запись добавлена", "success"); }} disabled={!form.date || !form.service}>СОХРАНИТЬ</PBtn><GBtn onClick={() => setAdding(false)}>ОТМЕНА</GBtn></div>
        </Card> : <PBtn full onClick={() => setAdding(true)}>+ НОВАЯ ЗАПИСЬ В СЕРВИС</PBtn>}
        {!upcoming.length && !adding && <Card style={{ textAlign: "center", padding: "32px", background: `${C.b}06`, borderColor: `${C.b}20` }}><div style={{ fontSize: 32, marginBottom: 12 }}>📅</div><div style={{ fontSize: 14, color: C.t2, fontFamily: S }}>Нет предстоящих записей</div></Card>}
        {past.length > 0 && <Card><Lbl>ИСТОРИЯ</Lbl>{past.slice(0, 5).map((a, i) => <div key={a.id} style={{ display: "flex", gap: 10, padding: "9px 0", borderBottom: i < 4 ? `1px solid ${C.border}` : "none", alignItems: "center", opacity: .6 }}><div style={{ minWidth: 42, textAlign: "center" }}><div style={{ fontSize: 14, fontWeight: 700, color: C.t2, fontFamily: M }}>{new Date(a.date).getDate()}</div><div style={{ fontSize: 10, color: C.t2, fontFamily: M }}>{new Date(a.date).toLocaleString("ru", { month: "short" })}</div></div><div style={{ flex: 1 }}><div style={{ fontSize: 13, color: C.t2, fontFamily: S }}>{a.service}</div><div style={{ fontSize: 11, color: C.t2, fontFamily: S }}>{a.shop || "—"}</div></div><button onClick={() => setAppts(p => p.filter(x => x.id !== a.id))} style={{ background: "none", border: "none", color: C.t2, cursor: "pointer", fontSize: 14 }}>×</button></div>)}</Card>}
      </div>
    );
  }
  ...
  if (sub === "tires") {
    ...
    const savePressure = () => { const p = parseFloat(val); if (isNaN(p) || p < 0.5 || p > 5) { toast("0.5–5.0 бар", "error"); return; } setTires(prev => ({ ...prev, [editing]: { ...prev[editing], pressure: p, lastCheck: new Date().toISOString().slice(0, 10) } })); toast(`${tires[editing]?.pos}: ${p} бар`, "success"); setEditing(null); setVal(""); };
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <BackHeader title="Давление в шинах" onBack={() => setSub(null)} />
        <Card>
          <Lbl>СХЕМА КОЛЁС (нажмите для обновления)</Lbl>
          <div style={{ display: "flex", justifyContent: "center" }}>
            <svg viewBox="0 0 200 170" style={{ width: "100%", maxWidth: 280, height: "auto" }}>
              ...
              {positions.map(({ key, x, y, label }) => { const t = tires[key]; const st = tirePressureStatus(t.pressure); return <g key={key} style={{ cursor: "pointer" }} onClick={() => { setEditing(key); setVal(String(t.pressure)); }}><circle cx={x + 20} cy={y + 20} r="18" fill={st.color + "25"} stroke={st.color} strokeWidth="2" /><text x={x + 20} y={y + 16} textAnchor="middle" fontSize="11" fontWeight="700" fill={st.color} fontFamily="monospace">{t.pressure}</text><text x={x + 20} y={y + 27} textAnchor="middle" fontSize="7" fill={C.t2} fontFamily="monospace">бар</text><text x={x + 20} y={y + 38} textAnchor="middle" fontSize="7" fill={C.t2} fontFamily="monospace">{label}</text></g>; })}
            </svg>
          </div>
        </Card>
        {editing && <Card style={{ borderLeft: `3px solid ${C.o}`, animation: "fadeUp .2s ease" }}>
          <Lbl>ОБНОВИТЬ — {tires[editing]?.pos}</Lbl>
          <div style={{ display: "flex", gap: 8, marginBottom: 8 }}><Input type="number" value={val} onChange={e => setVal(e.target.value)} placeholder="2.2" style={{ flex: 1, fontSize: 18, textAlign: "center" }} onKeyDown={e => e.key === "Enter" && savePressure()} autoFocus /><PBtn onClick={savePressure} color={C.g} style={{ padding: "10px 16px", fontSize: 14 }}>✓</PBtn><GBtn onClick={() => setEditing(null)}>✗</GBtn></div>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{[1.8, 2.0, 2.1, 2.2, 2.3, 2.4, 2.5].map(v => <button key={v} onClick={() => setVal(String(v))} style={{ padding: "4px 10px", fontSize: 12, fontFamily: M, background: C.oG, color: C.o, border: `1px solid ${C.o}30`, borderRadius: 6, cursor: "pointer" }}>{v}</button>)}</div>
        </Card>}
        <Card><Lbl>ДЕТАЛИ</Lbl>
          {Object.entries(tires).map(([key, t], i, arr) => { const st = tirePressureStatus(t.pressure); return <div key={key} style={{ display: "flex", gap: 12, padding: "10px 0", borderBottom: i < arr.length - 1 ? `1px solid ${C.border}` : "none", alignItems: "center" }}><div style={{ width: 36, height: 36, borderRadius: "50%", background: st.color + "15", border: `2px solid ${st.color}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><span style={{ fontSize: 13, fontWeight: 700, color: st.color, fontFamily: M }}>{t.pressure}</span></div><div style={{ flex: 1 }}><div style={{ fontSize: 13, color: C.t, fontFamily: S, fontWeight: 500 }}>{t.pos}</div><div style={{ fontSize: 11, color: C.t2, fontFamily: M }}>норма: {t.target} · {t.lastCheck || "не проверялось"}</div></div><Pill color={st.color}>{st.label}</Pill><button onClick={() => { setEditing(key); setVal(String(t.pressure)); }} style={{ background: "none", border: "none", color: C.t2, cursor: "pointer", fontSize: 16, padding: "0 4px" }}>✎</button></div>; })}
        </Card>
      </div>
    );
  }
  ...
  if (sub === "service") {
    ...
    const search = async (qv) => { const query = (qv || q).trim(); if (!query) return; setLoading(true); setData(null); try { const t = await ai(FIND_SYS2, `${query} для ${car.make} ${car.model} ${car.year} в ${geo?.city || "вашем городе"}`); setData(JSON.parse(t)); } catch { toast("Ошибка поиска", "error"); } setLoading(false); };
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
  ...
  if (sub === "tips") {
    ...
    const [msgs, setMsgs] = useState([{ role: "j", text: `Привет! Я Джарвис. Задайте любой вопрос о ${car.make} ${car.model}.` }]);
    const bottom = useRef(null);
    const send = async (q) => { const msg = (q || input).trim(); if (!msg || loading) return; setMsgs(p => [...p, { role: "u", text: msg }]); setInput(""); setLoading(true); try { const t = await ai(TIPS_SYS2, `${car.make} ${car.model} ${car.year}, ${car.mileage?.toLocaleString()} км. ${msg}`, 600); setMsgs(p => [...p, { role: "j", text: t }]); } catch { setMsgs(p => [...p, { role: "j", text: "Ошибка." }]); } setLoading(false); setTimeout(() => bottom.current?.scrollIntoView({ behavior: "smooth" }), 100); };
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <BackHeader title="Советы Джарвиса" onBack={() => setSub(null)} />
  ...
  if (sub === "sos") {
    ...
    const G = { dtb: { title: "ДТП", color: C.r, steps: ["Включите аварийную сигнализацию", "При пострадавших — немедленно 112", "Знак: 15м в городе, 30м на трассе", "Фото: место, повреждения, номера всех", "Данные всех участников + страховка", "Европротокол при согласии без пострадавших", "Страховую уведомить за 5 дней"] }, flat: { title: "Пробитое колесо", color: C.y, steps: ["Плавно снижайте скорость", "Аварийка, обочина справа", "Знак аварийной остановки", "Ручник, ровная поверхность", "Ослабьте болты на земле (1 оборот)", "Домкрат под точку кузова", "Колесо, болты крест-накрест", "Через 50 км проверьте затяжку"] }, bat: { title: "АКБ — прикурить", color: C.y, steps: ["Красный: + ваш → + донор", "Чёрный: − донор → металл кузова", "Заведите донор, ждите 5 мин", "Заводите свой автомобиль", "Провода в обратном порядке", "30 мин езды для зарядки"] }, heat: { title: "Перегрев", color: C.r, steps: ["Немедленно остановитесь!", "Заглушите мотор, аварийка", "НЕ открывайте крышку — кипяток!", "Ждите 30 минут до остывания", "Долейте охлаждающую жидкость", "Утечка → только эвакуатор"] } };
  ...
  const menu = [{ id: "appts", icon: "📅", label: "ЗАПИСИ В СЕРВИС", sub: `${appts.filter(a => new Date(a.date) >= new Date()).length} предстоящих` }, { id: "tires", icon: "🛞", label: "ДАВЛЕНИЕ ШИН", sub: `${Object.values(tires).filter(t => tirePressureStatus(t.pressure).label !== "НОРМА").length} требуют внимания` }, { id: "service", icon: "🔍", label: "НАЙТИ СЕРВИС", sub: geo?.city ? `Рядом в ${geo.city}` : "Demo-режим" }, { id: "tips", icon: "💡", label: "СОВЕТЫ ДЖАРВИСА", sub: "ИИ-ответы на вопросы" }, { id: "sos", icon: "🆘", label: "ЭКСТРЕННАЯ ПОМОЩЬ", sub: "ДТП, поломка" }, { id: "docs", icon: "📄", label: "МОИ ДОКУМЕНТЫ", sub: "ОСАГО, КАСКО, техосмотр" }, { id: "profile", icon: "🚗", label: "МОЙ АВТОМОБИЛЬ", sub: "VIN, гос. номер, данные" }];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <button onClick={() => { if (window.confirm("Сбросить все данные?")) { "ja8_car,ja8_services,ja8_diagHistory,ja8_expenses,ja8_fuel,ja8_body,ja8_onboarded,ja8_name,ja8_geo,ja8_tires,ja8_appts,ja8_docs,ja8_lastTipTime,ja8_tipIdx".split(",").forEach(k => localStorage.removeItem(k)); window.location.reload(); } }} style={{ padding: "12px", background: "transparent", border: `1px solid ${C.r}30`, color: C.r, borderRadius: 10, fontSize: 11, fontFamily: M, cursor: "pointer", letterSpacing: 1 }}>🔄 СБРОСИТЬ ВСЕ ДАННЫЕ</button>
      <Card style={{ background: `${C.o}06`, borderColor: `${C.o}15`, textAlign: "center", padding: "18px" }}>
        <div style={{ fontSize: 22, marginBottom: 6 }}>⚙</div>
        <div style={{ fontSize: 10, fontWeight: 700, color: C.o, fontFamily: M, letterSpacing: 1, marginBottom: 4 }}>JARVIS AUTO BETA v8</div>
        <div style={{ fontSize: 11, color: C.t2, fontFamily: S }}>Уведомления · Аналитика · Быстрый расход · Поиск</div>
      </Card>
    </div>
  );
}
...
/* ── ROOT ────────────────────────────────────────────────────────── */
const TABS = [{ id: "home", icon: "⌂", label: "ГЛАВНАЯ" }, { id: "diag", icon: "⚙", label: "ДИАГНОЗ" }, { id: "body", icon: "🚗", label: "КУЗОВ" }, { id: "maintain", icon: "≡", label: "СЕРВИС" }, { id: "analytics", icon: "📊", label: "АНАЛИТИКА" }, { id: "finance", icon: "₽", label: "РАСХОДЫ" }, { id: "more", icon: "···", label: "ЕЩЁ" }];
...
export default function JarvisApp() {
  ...
  const [docs] = usePersist("docs", DEF_DOCS);
  ...
  // Notifications
  const allNotifs = useMemo(() => buildNotifications(services, docs, tires, appts, car), [services, docs, tires, appts, car]);
  ...
  if (!onboarded) {
    ...
    const [fc, setFc] = useState(DEF_CAR);
    const steps = [
      { icon: "⚙", title: "JARVIS AUTO v8", sub: "Аналитика · Уведомления · Быстрый расход", content: <div style={{ display: "flex", flexDirection: "column", gap: 12 }}><div style={{ fontSize: 14, color: C.t2, fontFamily: S, lineHeight: 1.7, textAlign: "center" }}>Полная платформа управления автомобилем с ИИ.</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>{[["⚙", "Диагностика", "OBD + ИИ"], ["📊", "Аналитика", "Стоимость/км"], ["🔔", "Уведомления", "Центр алертов"], ["📅", "Записи", "К мастеру"]].map(([ic, t, s]) => <div key={t} style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 10, padding: "12px", textAlign: "center" }}><div style={{ fontSize: 20, marginBottom: 4 }}>{ic}</div><div style={{ fontSize: 10, color: C.t, fontFamily: M, marginBottom: 2 }}>{t}</div><div style={{ fontSize: 10, color: C.t2, fontFamily: S }}>{s}</div></div>)}</div></div> },
      ...
      {
        icon: "🚗", title: "ВАШ АВТОМОБИЛЬ", sub: "Базовые данные",
    ...
    return (
      <div style={{ minHeight: "100vh", background: C.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 20px", fontFamily: M }}>
        ...
        <div style={{ width: "100%", maxWidth: 420 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, justifyContent: "center", marginBottom: 36 }}><div style={{ width: 44, height: 44, borderRadius: "50%", background: C.o, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 0 24px ${C.o}50` }}><span style={{ fontSize: 20 }}>⚙</span></div><div><div style={{ fontSize: 18, fontWeight: 700, color: C.t }}>JARVIS AUTO</div><div style={{ fontSize: 9, color: C.t2, letterSpacing: 2 }}>v8 · AI AUTOMOTIVE PLATFORM</div></div></div>
          ...
          <div key={step} style={{ background: C.card, border: `1px solid ${C.border2}`, borderRadius: 16, padding: "28px 24px", animation: "fadeUp .35s ease" }}>
            ...
            {cur.content}
            <PBtn full onClick={() => { if (!valid[step]) return; if (step < steps.length - 1) { setStep(s => s + 1); } else { setName(nm.trim() || "Водитель"); setCar({ ...fc, year: +fc.year, mileage: +fc.mileage }); setOnboarded(true); } }} disabled={!valid[step]} style={{ marginTop: 18, padding: "14px" }}>{step < steps.length - 1 ? "ПРОДОЛЖИТЬ →" : "ВОЙТИ В ДЖАРВИС ⚙"}</PBtn>
  ...
  const screens = {
    home: <Dashboard name={name} car={car} services={services} diagHistory={diagHistory} expenses={expenses} fuel={fuel} body={body} tires={tires} geo={geoState.geo} docs={docs} appts={appts} setTab={setTab} toast={toast} onRequestGeo={geoState.request} geoLoading={geoState.loading} onQuickAdd={() => setShowQuickAdd(true)} onOpenNotifs={() => setShowNotifs(true)} notifCount={notifCount} />,
    ...
    analytics: <Analytics expenses={expenses} fuel={fuel} services={services} car={car} diagHistory={diagHistory} />,
    finance: <Finance expenses={expenses} setExpenses={setExpenses} fuel={fuel} setFuel={setFuel} car={car} toast={toast} />,
    more: <More car={car} name={name} toast={toast} setCar={setCar} expenses={expenses} setExpenses={setExpenses} fuel={fuel} setFuel={setFuel} body={body} geo={geoState.geo} onRequestGeo={geoState.request} geoLoading={geoState.loading} appts={appts} setAppts={setAppts} tires={tires} setTires={setTires} setServices={setServices} services={services} docs={docs} />,
  };
  ...
  return (
    <div style={{ fontFamily: M, background: C.bg, minHeight: "100vh", display: "flex", flexDirection: "column", maxWidth: 640, margin: "0 auto" }}>
      ...
      {showNotifs && <NotificationCenter services={services} docs={docs} tires={tires} appts={appts} car={car} setTab={setTab} onClose={() => setShowNotifs(false)} />}
      {showQuickAdd && <QuickAdd onAdd={exp => { setExpenses(p => [exp, ...p]); toast(`Добавлено: ${rub(exp.amount)}`, "success"); setShowQuickAdd(false); }} onClose={() => setShowQuickAdd(false)} />}
      ...
      <div style={{ flex: 1, padding: "18px 16px 94px", overflowY: "auto" }}>
        <div key={tab} style={{ animation: "fadeUp .25s ease" }}>{screens[tab] || screens.home}</div>
      </div>

      {/* Bottom nav — 7 tabs, scroll if needed */}
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 640, background: C.bg2, borderTop: `1px solid ${C.border}`, display: "flex", padding: "8px 0 18px", zIndex: 40, overflowX: "auto" }}>
        {TABS.map(t => {
          const active = tab === t.id;
          const badge = t.id === "maintain" ? overdueCount : t.id === "body" ? bodyIssues : t.id === "more" ? upcomingAppts : t.id === "home" ? notifCount : 0;
          return (
            <button key={t.id} onClick={() => setTab(t.id)}
              style={{ flex: 1, minWidth: 52, display: "flex", flexDirection: "column", alignItems: "center", gap: 3, background: "none", border: "none", cursor: "pointer", color: active ? C.o : C.t2, transition: "color .15s", position: "relative", padding: "4px 4px" }}>
              {active && <div style={{ position: "absolute", top: -8, left: "50%", transform: "translateX(-50%)", width: 24, height: 2, background: C.o, borderRadius: 1 }} />}
              <span style={{ fontSize: t.id === "body" ? 14 : 17, lineHeight: 1 }}>{t.icon}</span>
              <span style={{ fontSize: 7, letterSpacing: .5, fontFamily: M }}>{t.label}</span>
